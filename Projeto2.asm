...
MsgNJogadores: .asciiz "\nQuanto jogadores v�o jogar?\n"
OpcaoCol: .asciiz "Escolha uma coluna para colucar a pe�a\n"
ErroOpcaoCol:.asciiz "#ERRO#\nIntroduza um valor entre  "
mensagemErroColuna: .asciiz "Esta coluna ja esta cheia!\nInserir outra\n"
MsgErroNPlayers: .asciiz "O numero minimo de jogadores para jogar � 2, introduza um numero >1\n"
...
MsgPontosErro: .asciiz "\n� necessario jogar pelo menos uma vez para dar display aos pontos\n"
EmpateMsg: .asciiz "O jogo ficou empatado, n�o h� vencedores\n"
mat: .word 0
nPlayers: .word 0
BytesCelulas: .word 0
...
vDesenho: .word 1:15				#Vetor que auxiliar o desenho (para amontoar as pe�as)
CorJogador: .word 0
Vencedor: .word 0
...
espacos: .word 0
MSGFim: .asciiz "\nPrograma terminado\n"
pos: .word 0						#Apenas ajuda a calcular bytes da coluna onde foi inserida a pe�a
endline: .asciiz	"\n"				#Paragrafo entre as linhas
tab:	.asciiz	"\t"					#Espa�o entre os numeros
separador: .asciiz "-----------------------------------------"
.align 2
frameBuffer: 
	.space 0x80000
...
menu:
	...
	beq $v0,'f', exit
	j ErroMenu					#Caso n�o introduza nenhuma das op��es salta para o funcao ErroMenu
...
ErroMenu:
	li $v0, 4
	la , $a0, menuMsgErro				#Print � string de rro
	syscall
...
AskNPlayers:
	li $v0, 4
	la $a0, MsgNJogadores				#Print � string
	syscall
	...
	li $v0, 9					#Alocar espa�o para o tamanho da matriz	
	syscall						#Guarda o numero total de espa�o.
...
ErroNJogadores:
	...
	syscall
	j AskNPlayers					#Volta para a fun��o
...
reset_player:
		...
		addi $a0, $0, 1				#Come�a o count a 1, pois nao existe jogar 0
...
PlayJogador:
	la $s0, mat
	lw $s0,0($s0)
							#Come�a a contar as jogadas
	lw $t1, espacos					#Load dos espacos(celulas da mat+1)
	lw $t2, countjogadas				#Load do numero de jogadas
	beq $t1, $t2 ,JogoEmpatado			#Se foram iguais, quer dizer que at� agr nao existiu nenhum vencedor e matriz ja est� toda preenchila, logo o jogo empatou
	li  $v0,4   				
	la  $a0,MsgJogadorPlay1				#Print � string	
	syscall 
	lw $t0, count_jogador				#Print do jogador que est� a jogar	
	li $v0,1
	move $a0,$t0
	syscall
	li  $v0,4   				
	la  $a0,MsgJogadorPlay2				#Print � string
	syscall 
	li $v0, 4
	la $a0, endline					#Print a um \n
	syscall
	li $v0, 4 					
	la $a0,OpcaoCol					#Print � string
	syscall
	li $v0, 5					#L� o numero colocado
	syscall
	addi $v0,$v0,-1 				#Subtrai esse numero por 1, pois na programa��o 0 tambem conta
        mul $t0,$v0,4					#Multiplica por 4 para saber onde se encontra na matriz
        sw $t0, pos					#Da deve dessa posi��o ( para auxiliar em funcoes futuras)
        move $a0,$v0					#Passa para a0
	jal Validacao 					#Salta para a fun��o de valida��o do numero.
	beq $v0,0, ColunaInvalidaPlayer			#Se o valor retornado da fun��o "Validacao" for 0, ele ira imprimir msg de erro e volta a pedir outra coluna.
	lw $t5,count_jogador				#t5 ser� a pe�a , onde o jogador x ter� a pe�a (x)
	jal VerificaoColuna 				#Salta para a fun��o para verificar se a celula � valida ou nao
	sw $t5,0($v0) 					#Coloca esse valor na matriz na coluna que o utilizador colocou
	jal ilustracao					#Salta para a fun��o de impressao do numero em cima das colunas
	jal mat_print					#Mostra o estado da matriz
	j peca
	FIMDESENHO:
	jal checkVertical				#V� se existe algum 4 em linha na vertical
	beq $v0, 1 , PlayerWin				#Caso existe player x ganhou	
	jal checkhorizontal				#V� se existe algum 4 em linha na horizontal
	beq $v0, 1 , PlayerWin				#Caso existe player x ganhou	
	jal check1stdiagonal				#V� se existe algum 4 em linha na primeira diagonal
	beq $v0, 1 , PlayerWin				#Caso existe player x ganhou
	jal check2nddiagonal				#V� se existe algum 4 em linha na segunda diagonal
	beq $v0, 1 , PlayerWin				#Caso existe player x ganhou
	jal counter_jogadas
	jal counter_players				#Muda para o proximo jogador
	lw $t3, nPlayers				#Da load do numero de jogadores
	beq $t3,$a1,reset_player			#Caso esse numero seja igual ao jogador atual, ent�o ir� dar reset para come�ar de novo o primeiro jogador a jogar
	
	j PlayJogador					#Fica em loop at� haver um vencedor


counter_jogadas:
		lw $t0, countjogadas			#Load do numero de jogadas
		add $t0, $t0, 1				#Adiciona +1
		sw $t0, countjogadas			#D� save
		jr $ra					#Retorna
...
VerificaoColuna:
	lw $t0, pos					#Da load da coluna onde foi colocado (x4 bytes)
        ...
        add $s0,$s0,$t0      
        loop1:
         	lw $t2 ,0($s0)				#t2 fica com o valor que existe na posi�ao da matriz			
                beq $t2 ,0 ,end1			#Se for 0, entao vai para o fim da funcao
                lw $t3 , maiorcoluna			#Se nao for 0, ir� andar (4xNcolunas) bytes
                mul  $t3, $t3, 4			#Ou seja, vai exatamente para a posi��o de cima para verificar se � possivel colocar a pe�a l�      							
         	add $s0,$s0, $t3			#linha a cima	
         	j loop1					#Volta para o loop
        end1:
        	addi $v0,$s0,0				#$v0 ser� a localiza�ao da pe�a
                jr $ra					#return v0;		
...
Validacao:
	...
	bgt $a0,$t2, Invalido 				#Se o numero introduzido for maior que o numero de colunas total (-1), salta para funcao "Invalido"
        ...
        lw $t6, 0($s0) 					#Carrega o valor que est� na celula da matriz
    	bne $t6 ,0 , ColunaCheia			#Se o valor que est� na celula ultima celula da coluna for diferente de 0, salta para a funcao ColunaCheia
        li $v0,1 					#Se o valor for 0, a fun��o retorna 1
        jr $ra 						#return 1;
              		
Invalido:
        li $v0,0 					#Se a celula estiver preenchida (ou seja diferente de 0) a fun��o retorna 0
        jr $ra 						#return 0;
...
ColunaCheia:
	li $v0, 4
	la $a0, mensagemErroColuna 			#Imprime que a coluna est� cheia e salta para o jogador x
	syscall
	

	j PlayJogador	

ilustracao:						#Fun��o que apenas mostra o layout do tabuleiro para te ruma melhor perce��o do numero de cada colun
	
		addi $t0, $0, 1				#t0 come�a a 1
...
checkVertical:						#Utiliza o mesmo mecanismo que a funcao de print matriz so que nao analisa ao contra e sim de cima para baixo
	...
	mat_scan2222: 				
		...
		sll 	$t8, $a2, 2			#t8 sera o deslocador entre as posi�oes, dependendo do tamanho da matriz t8 ter� o seu valor
		
		lw 	$a0, 0($t5)			#Load do numero na posicao base
		beq 	$a0, 0, NextPosicion1		#Se for 0 salta para o proximo j
		
		add $t5, $t5 , $t8			#t8 ser� o numero de bytes necessario para saber o valor da pe�a numa dada posicao
		lw 	$t9, ($t5)			#load do numero
		beq 	$t9, 0, NextPosicion1		#caso seja 0 salta para o proximo j
		
		add $t5, $t5 , $t8			#agora t5 ter� o valor da posi��o para a 3 compara�ao	
		lw 	$t2, ($t5)			#load do numero
		beq 	$t2, 0,NextPosicion1		#Caso seja 0 salta para proximo j
		
		add $t5, $t5 , $t8			#agora t5 ter� o valor da posicao para a 4 comparacao		
...
checkhorizontal:					#Esta fun��o funciona exatamente como a de cima, apenas nao precisa de um registo deslocador , porque apenas existe 4 em linha na horizontal quanto estao espa�ados de 4 em 4, nao dependendo do tamanho da matriz
...
check1stdiagonal:						#Funciona exatamente comoo a de cima, so que agora a formula ser� (NColunas*4)-4
...
PlayerWin:
	...
	syscall
	li $v0, 1
	lw $a0, count_jogador						#Print ao jogador que est� na memoria, ou seja o vencedor
	syscall
	li $v0, 4
	la $a0, jogadorwin						#Print � string
	syscall
	lw $t0, count_jogador						
	sw $t0, Vencedor						#Guarda o numero do vencedor (para fun��es futuras)
...
stats:
	...
	addi $t2, $t2, -4						#Subtrai 4 , devido � posi��o 0
	sw $t2, posWinner						#Guarda esse valor na memoria
	add $t3, $0, $0							#t3=i
	add $t5, $0, $0							#t5 ser� a posi�ao do array e comeca no 0
	...
	L:
		...
		beq $t5, $t2, EE					#Caso a posi�ao do array seja a do vencendor, salta para EE
		lw $t4,vPoints($t5)					#Caso ambas as condi�oes em cima sejam respeitadas, da load do numero de pontos na posicao do array
		...
		H:							#Funcao que auxiliar funcao "ZeroPontos"
			...
			EE:						#Funcao que soma os pontos do vencedor e a vitoria
				...
				addi $t0,$t0,-4				#Tira 4 devido � primeira posi��o
	...
	E:
	lw $t6, vWins($t0)						#Da load da posi�ao do array do vencedor
	addi $t6, $t6, 1						#Adiciona mais uma vitoria
	sw $t6, vWins($t0)						#D� save
	jr $ra
...
printPoints:
	...
	LOOOP: 
		...
		li $v0, 4
		la $a0, doispontos					#print � string
		syscall
	
		
		lw $t2, vPoints($t4)					#load do numero que est� na posicao do jogador no array
		...
		addi $t4, $t4, 4						#salta para as proximas posi�oes do array
		sw $t0, count_jogador
...
AskRepetir:
	li $v0, 4
	la $a0, Repetir							#print � string
	syscall
	li $v0, 5							#Le o valor
	syscall
	beq $v0, 1, RepetirJogo						#Se for 1 repete jogo com mesmo numero de jogadores
	beq $v0, 0, main						#Se for 0 pergunta um novo numero de jogadores
	beq $v0, 2, menu						#Se for 2 ir� voltar ao menu para futuras utilizacoes
	j ErroAskRepetir
...
####################################################################################################################################
inicioX:
	lw $t1, GrelhaX					#Tamanho do espa�amento estre as colunas
	addiu $t1, $0, 0
	sw $t1, GrelhaX					
...
DesenhoTabX:
	...
	lw $t8, GrelhaX					#Da load do espa�amento
	addu $t8, $t8,$t7				#Adiciona esse valor a ele propio mais o espa�amento (t7)
...
inicioY:
	lw $t1, GrelhaY					#Tamanho do espa�amento entre as linhas
...
DesenhoTabY:
	...
	addu $t8, $t8,$t7				#Adiciona o espa�amento
